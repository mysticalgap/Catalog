...
namespace Repository
{
    public class AuthorRepository : RepositoryBase<Author>, IAuthorRepository
    {
        ...
        {
            var authors = await FindByCondition(e => e.BookId.Equals(bookId), trackChanges).Search(authorParameters.SearchTerm).Sort(authorParameters.OrderBy).ToListAsync();
            return PagedList<Author>.ToPagedList(authors, authorParameters.PageNumber,authorParameters.PageSize);
        }
        public async Task<Author> GetAuthorAsync(Guid bookId, Guid id, bool trackChanges) => await FindByCondition(a => a.BookId.Equals(bookId) && a.Id.Equals(id),trackChanges).SingleOrDefaultAsync();
